"""
Description: This project is a backup car and the project for 
EE 542, Spring 2024. The program leverages openCV to open the stream
created by the ESP on its own network. It uses yolov5 to detect
people and cars. 

The idea was it would run on a raspberry pi in the front of the car
and have a corresponding display. The esp would be powered indepently and 
live in the trunk and transmit to the rp

Unfornately my display's refresh rate was too poor

Author: Wyatt Colburn, wdcolbur@calpoly.edu
Github: wyattdcolburn@gmail.com

Room for improvement: There is like .3 second delay, yolo works so much 
better in yolo's detect.py. I think it could be the post processing I do by running the yolo 
ontop of the frame rather than do both at the same time.

Would love to have a better hud, that actually takes motor commands from my car.
One that looks more 3-d

Professor Danowitz: Thank you for all your support. This was a difficult quarter,
I wanted to do more, but life got in the way. C'est la vie
"""
import cv2
import numpy as np
import math
